...
export const race = (tasks: Array<Task>): Task =>
  Task((rej, res) => tasks.forEach(t => t.fork(rej, res)));

// series :: [Task e a] -> Task e [a]
export const series = (tasks: Array<Task>): Task<any, Array<any>> => tasks.reduce(
  (task, t) => task.chain((d: Array<any>) => t.map((x: any) => d.concat([x]))),
  Task.Resolved([]),
);

// parallel :: [Task e a] -> Task e [a]
export const parallel = (tasks: Array<Task>): Task<any, Array<any>> => Task((reject, resolve) => {
  let resolvedCount = 0;
